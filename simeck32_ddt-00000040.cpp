...
uint16_t const_simeck32[] = {0xfffd, 0xfffd, 0xfffd, 0xfffd,
...
{
    uint16_t ks_tmp[4]={0}, tmp;
    memcpy(ks_tmp, &key, sizeof(uint16_t) * m);

    ks[0] = ks_tmp[0];
    for (uint32_t i = 1; i < rounds; i++)
    {
        ks[i] = ks_tmp[1];
        tmp = (rol(ks_tmp[1], ALPHA) & rol(ks_tmp[1], BETA)) ^ rol(ks_tmp[1], GAMMA) ^ ks[i-1] ^ const_simeck32[i-1];
        ks_tmp[1] = ks_tmp[2];
        ks_tmp[2] = ks_tmp[3];
        ks_tmp[3] = tmp;
    }
...
//the following function calculates the probability of a xor-differential transition of one round of Simeck32 according to Stefan K̈olbl, Gregor Leander, and Tyge Tiessen
...
void calc_ddt(uint32_t in_diff, int num_rounds){
  ...
  for (int i = 0; i < num_rounds; i++){
	...
	printf("Specify 24 threads and execution time: %f\n\n", endTime - startTime);
    if ((i+1) >= 0)
    {
